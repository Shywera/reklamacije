...
:build
echo [QMS] Kreiram okruzenje i instaliram ovisnosti...
py -3 -m venv .venv
...
echo.
echo Pokrecem QMS server (WiFi mreza)...
echo   Lokalno:  http://localhost:8601
if defined IP echo   Mreza:    http://%IP%:8601
echo.
...
echo   netsh advfirewall firewall add rule name="QMS dev 8601" dir=in action=allow protocol=TCP localport=8601
echo.

.venv\Scripts\uvicorn app.main:app --host 0.0.0.0 --port 8601
pause
goto :eof
