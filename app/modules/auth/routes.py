...
def login_form(request: Request):
    if request.session.get("user_id"):
        return RedirectResponse("/reklamacije", status_code=303)
...
def korisnik_novi(request: Request, username: str = Form(""), ime: str = Form(""),
                  email: str = Form(""), lozinka: str = Form(""),
                  dozvole: list[str] = Form(default=[]), db: Session = Depends(get_db)):
    ...
    db.add(User(username=username, ime=ime.strip() or None, email=email.strip() or None,
...
def korisnik_uredi(request: Request, uid: int, ime: str = Form(""), email: str = Form(""),
