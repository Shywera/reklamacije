...
class User(Base):
    """Korisnik QMS-a. `dozvole` = CSV ključeva iz `security.PERMISSIONS` (admin = sve)."""
    ...
    dozvole: Mapped[str] = mapped_column(Text, default="")          # CSV: "unos,admin"
...
class AuditLog(Base):
    ...
    `predmet_id` veže zapis uz konkretnu reklamaciju (za timeline na detalju)."""
    ...
    predmet_id: Mapped[int | None] = mapped_column(Integer, index=True)   # FK reklamacija.id (soft)
